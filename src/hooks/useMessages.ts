...
export const useMessages = (conversationId: string | null) => {
  ...
  const getCurrentStateFromRefs = useCallback(() => {
    return {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message: Message) => {
      ...
      if (message.conversationId === conversationId) {
        // Always show system messages and polls regardless of sender
        ...
        console.log('📋 Message show logic:', { shouldShow, type: message.type, senderId: message.senderId, currentUserId: session?.user?.id })
        ...
        if (shouldShow) {
          setMessages(prev => {
            ...
            if (message.type === 'system') {
              ...
              console.log('📊 useMessages: Adding POLL message:', message.content.substring(0, 50))
              console.log('📊 useMessages: Poll details:', { 
                id: message.id, 
        ...
        console.log('❌ useMessages: Message for different conversation:', { messageConversation: message.conversationId, currentConversation: conversationId })
      }
    }
    ...
    const handlePollUpdated = (data: { pollId: string; poll: any }) => {
      console.log('🔌 Socket received poll-updated event:', {
        pollId: data.pollId,
        poll: data.poll ? 'Poll data present' : 'No poll data',
        conversationId: conversationId
      })
      
      setMessages(prev => prev.map(msg => {
        if (msg.type === 'poll' && msg.poll?.id === data.pollId) {
          console.log('📊 Updating poll in message:', msg.id, 'with new poll data')
          return {
            ...msg,
            poll: data.poll
          }
  ...
  // Handle immediate message addition (for current user's polls)
  useEffect(() => {
    const handleImmediateMessage = (event: CustomEvent) => {
      const message = event.detail
      if (message && message.conversationId === conversationId) {
        setMessages(prev => {
          // Prevent duplicate messages by checking for existing ID
          if (prev.some(m => m.id === message.id)) {
            console.log('📨 useMessages: Immediate duplicate message ignored:', message.id)
            return prev
          }
          
          console.log('📨 useMessages: Adding immediate message:', message.type, message.content?.substring(0, 50))
          return [...prev, message]
        })
      }
    }

    const handleImmediatePollUpdate = (event: CustomEvent) => {
      const { pollId, poll } = event.detail
      if (pollId && poll) {
        setMessages(prev => prev.map(msg => {
          if (msg.type === 'poll' && msg.poll?.id === pollId) {
            console.log('📊 useMessages: Immediate poll update:', pollId)
            return {
              ...msg,
              poll: poll
            }
    ...
    window.addEventListener('add-immediate-message', handleImmediateMessage as EventListener)
    window.addEventListener('update-immediate-poll', handleImmediatePollUpdate as EventListener)
    
    return () => {
      window.removeEventListener('add-immediate-message', handleImmediateMessage as EventListener)
      window.removeEventListener('update-immediate-poll', handleImmediatePollUpdate as EventListener)
    }
  }, [conversationId])
